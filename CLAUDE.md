...
- `ArbitrageScanner`'s worker no longer force-restarts every 4 hours — that `timeout 14400` wrapper was removed after a leak investigation (`docs/investigations/scanner-memory-leak.md`) fixed a concrete `ProxyService` issue (a new `HttpClient` was created on every proxy rotation instead of being reused), though the investigation didn't conclusively prove that was the original cause of the degradation. If you see unbounded memory/handle growth over a multi-hour run, that doc's "not yet ruled out" list is the place to continue from — don't just reintroduce the restart hack as a shortcut.
